...
export default function OpportunityStudio({
  ...
  function changeStudioStep(nextStep: StudioStep) {
    ...
    <div className="relative -m-6 flex min-h-[calc(100vh-96px)] flex-col bg-white">
        ...
        <div className="flex min-h-[64px] flex-wrap items-center gap-3 px-4">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="hidden items-center gap-1 rounded-lg bg-slate-100 p-1 md:flex">
              <button
                type="button"
                onClick={() => changeStudioStep("details")}
              ...
              <button
                type="button"
                onClick={() => changeStudioStep("pipeline")}
          ...
          {studioStep === "pipeline" ? (
            <>
              ...
              <button type="button" onClick={() => void publishWorkflow()} disabled={publishing} className="inline-flex min-h-[40px] items-center gap-2 rounded-lg bg-primary px-4 text-sm font-bold text-white shadow-sm disabled:opacity-50">
                <Icon name={publishing ? "progress_activity" : "publish"} className={`h-[18px] w-[18px] ${publishing ? "animate-spin" : ""}`} />
                Publish Workflow
              </button>
            </>
          ) : (
            <button type="button" onClick={() => changeStudioStep(studioStep === "details" ? "form" : "pipeline")} className="inline-flex min-h-[40px] items-center gap-2 rounded-lg bg-primary px-4 text-sm font-bold text-white shadow-sm">
      ...
      {studioStep === "details" ? (
        <SetupScreen
          ...
          onContinue={() => changeStudioStep("form")}
        />
      ) : studioStep === "form" ? (
        <ApplicationFormScreen
          ...
          onBack={() => changeStudioStep("details")}
      ...
      {studioStep === "pipeline" && (
        <div className="flex flex-1 flex-col lg:hidden">
          ...
          <nav className="mt-auto grid grid-cols-3 border-t border-slate-200 bg-white">
            <button type="button" onClick={() => changeStudioStep("form")} className="min-h-[56px] text-sm font-semibold text-slate-500">
              Form
            </button>
      ...
      {successOverlay && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/80 backdrop-blur-sm">
          <div className="rounded-xl border border-emerald-200 bg-white px-8 py-6 text-center shadow-lg">
            <Icon name="check_circle" className="mx-auto h-10 w-10 text-emerald-600" />
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
            <RailSection title={`Student Detail Fields (${detailFields.length})`}>
              <div className="flex flex-wrap gap-2">
                ...
                >
                  <Icon name="event" className="h-[18px] w-[18px]" />
                  Add Deadline
                </button>
              </div>
            ...
            <RailSection title="Student Summary">
              ...
              <div className="flex flex-wrap gap-2">
                <button type="button" onClick={onGenerateSummary} className="inline-flex min-h-[36px] items-center gap-1 text-sm font-semibold text-primary-dark">
                  <Icon name="auto_fix_high" className="h-[18px] w-[18px]" />
                  Generate Summary
                </button>
                <button
                  type="button"
    ...
    <main className="flex-1 overflow-y-auto bg-slate-50">
      <div className="mx-auto grid max-w-7xl gap-3 p-3 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <aside className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm xl:sticky xl:top-4 xl:self-start">
          <RailSection title="Sharing & eligibility">
            <p className="mb-3 text-xs leading-5 text-slate-500">
              Choose exactly who can see and apply to this opportunity. Group rules share it with a cohort mailbox; email rules share it with one person.
            </p>
            <div className="space-y-2">
    ...
    <aside className="rounded-xl border border-slate-200 bg-white shadow-sm lg:sticky lg:top-4 lg:self-start">
      <div className="border-b border-slate-100 px-4 py-3">
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-primary/10 p-2 text-primary-dark">
      ...
      <div className="space-y-3 p-3">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          >
            <Icon name={aiGenerating ? "progress_activity" : "auto_fix_high"} className={`h-[18px] w-[18px] ${aiGenerating ? "animate-spin" : ""}`} />
...
function PaletteButton({
  ...
  return (
    ...
    >
      <span className="mt-0.5 rounded-lg bg-slate-100 p-1.5 text-slate-600 group-hover:text-primary-dark">
...
function ChecklistRow({ done, label }: { done: boolean; label: string }) {
  return (
    <div className="flex items-center gap-2 text-sm">
      <Icon name={done ? "check_circle" : "radio_button_unchecked"} className={`h-[18px] w-[18px] ${done ? "text-emerald-600" : "text-slate-300"}`} />
