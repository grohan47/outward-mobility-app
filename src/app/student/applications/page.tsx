...
export default function MyApplications() {
  ...
  const columns = [
    ...
    },
    {
      key: "date",
      header: "Last Updated",
      cell: (item: ApplicationListItem) => <span className="text-sm text-slate-500">{new Date(item.updated_at).toLocaleDateString()}</span>,
    },
    {
      key: "actions",
      header: "Actions",
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <Table
            ...
            onRowClick={(item) => router.push(`/student/applications/${item.id}`)}
