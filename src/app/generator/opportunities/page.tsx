    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-6">
          {items.map((opp) => (
            <Card key={opp.id} className="group hover:-translate-y-1 hover:shadow-lg transition-all duration-300 border border-slate-200 hover:border-primary/30 flex flex-col">
              <div className="flex-1">
                <div className="flex items-start justify-between mb-4">
                  <Badge variant="neutral" icon="pin_drop">{opp.destination}</Badge>
              ...
              <div className="mt-8 pt-4 border-t border-slate-100">
                <Button className="w-full" icon="arrow_forward">Apply Now</Button>
              </div>
            </Card>
          ))}
        </div>
